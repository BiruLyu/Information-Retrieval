...
include 'snippetGenerator/snippetGenerator.php';
...
?>
  <div id="searchArea">
    <div class="areaTitle"><i>Search Engine Using Solr</i></div>
    <div class="formPart">
        <form method="post" action="<?php echo htmlspecialchars($_SERVER[" PHP_SELF "]);?>">
            <div>Search Keyword
                <input id="q" name="q" type="text" size="20" name="keyword" value="<?php echo htmlspecialchars($query, ENT_QUOTES, 'utf-8'); ?>" oninput="setCustomValidity('')" required>
            </div>
            <div>Ranking Type
                <select id="typeSelector" name="type" onchange="addLocation()" style="margin-left:20px">
                    <option value="solr" <?php if(htmlspecialchars($type,ENT_QUOTES, 'utf-8') =="solr" ) echo "selected";?>>Solr Ranking</option>
                    <option value="pagerank" <?php if(htmlspecialchars($type,ENT_QUOTES, 'utf-8') =="pagerank" ) echo "selected";?>>PageRank Ranking</option>
                </select>
            </div>
            <div style="padding-left:110">
                <input type="submit" value="submit" name="submit">
                <input type="reset" value="Clear" name="clear" onclick="clearForm()"> </div>
        </form>
    </div>
</div>
...
<div class="res">
      ...
      if($query){
        ...
        if($is_spell_error){
          ...
          ?>
          <div style="padding: 10px">Did you mean: <a href="query.php?type=solr&q=<?=$correct_terms?>"><?= $correct_terms; ?></a></div>
          <?php
        }
      }
    ?>
...
?>
      <td>
        <table>
          <tr style="text-align: center; background: grey; color: white;">
            <td colspan="2"><b><?php if ($type == "solr") {echo "Solr Ranking ";} else {echo "pageRank Ranking ";} echo "#"; echo $i + 1; ?></b></td>
          </tr>
      ...
      <?php
            ...
            <tr>
                <th>description</th>
                <td style="color:gray; font-size:14px" ><?php echo htmlspecialchars($doc->description, ENT_NOQUOTES, 'utf-8'); ?></td>
            </tr>
            <tr>
                <th>snippet</th>
                <td><?php 
